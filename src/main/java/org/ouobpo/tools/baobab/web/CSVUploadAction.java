...
public class CSVUploadAction extends ActionBase {
    ...
    private List<Book> csv2books(String csv) {
        ...
        for (String line : lines) {
            ...
            calendar.clear();
            calendar.set(Integer.parseInt(fields[0].trim()), Integer.parseInt(fields[1].trim()) - 1,
                    Integer.parseInt(fields[2].trim()));
            String title = fields[3].trim();
            String authors = fields[4].trim();
            String publisher = fields[5].trim();
            int price = Integer.parseInt(fields[6].startsWith("\\") ? fields[6].substring(1).trim() : fields[6].trim());
            books.add(new Book(-1, title, authors, publisher, calendar.getTime(), price, null));
        }
        return books;
    }
